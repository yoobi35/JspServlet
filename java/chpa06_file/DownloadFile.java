...
public class DownloadFile extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  // ---- 다운로드 응답 헤더 구성
	  String originFilename = URLEncoder.encode(filename.substring(filename.indexOf("_")+1), "UTF-8").replace("\\+", "%20");
	  ...
	  response.setHeader("Contetn-Length", file.length()+""); // ---- 다운로드 해 줄 파일 크기 
	  
	  
	  // ---- 서버에 저장도니 파일을 읽어서 그대로 응답하기 
	  try(

...
public class BoardServiceImpl implements BoardService {
  ...
  public ActionForward getBoards(HttpServletRequest request) {
    ...
    // ---- list.jsp로 forward(request를 전달하므로 request에 저장한 값도 전달됩니
  ...
  public ActionForward registBoard(HttpServletRequest request) {
    ...
    // ---- uid, title, content 값으로 BoardDTO 객체생성
  ...
  public ActionForward modifyBoard(HttpServletRequest request) {
    ...
    // ---- form으로 부터 재출된 bid, title, content 받기
    int bid = Integer.parseInt(request.getParameter("bid"));
    String title = request.getParameter("title");
    String content = request.getParameter("content");
    
    
    // ---- bid, title, content를 이용해 BoardDTO 생성
    BoardDTO board = new BoardDTO();
    board.setBid(bid);
    board.setTitle(title);
    board.setContent(content);
    ...
    // ---- 수정 결과에 따라 이동할 결로 결정 ( 성공하면 detail.jsp / 실패하면 modify.jsp )
    String view = null;
    if(count ==1) {
